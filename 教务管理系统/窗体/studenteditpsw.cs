...
namespace 教务管理系统.窗体
{
    public partial class studenteditpsw : Form
    {
        /// <summary>
        /// 当前登录的学生信息
        /// </summary>
        ...
        public studenteditpsw(StudentBean student, ClassBean classname)
        {
            InitializeComponent();
            this.ControlBox = false;   // 设置不出现关闭按钮
                                       //
            this.student = new StudentDao().FindById(new StudentBean(student.Id));
        }
        ...
        {
            string sqlStr = "SELECT pwd FROM [dbo].[student] WHERE pwd='"+textBox1.Text+"' and id="+student.Id;
          //  string sqlStr = "UPDATE student_course SET courseid=(select id from course WHERE Cname='" + comboBox1.Text + "'),studentid=" + student.Id + " WHERE studentid=" + student.Id;
            ...
            if (dataTable.Rows.Count > 0)
            {
                ...
                {
                    string sqlStr1 = "UPDATE student SET pwd='" + textBox2.Text + "' WHERE id=" + student.Id;
